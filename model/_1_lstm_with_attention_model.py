...
def seq2seq_with_attention(max_text_len, corpus_size, max_summary_len, corpus_size_answer, **kwargs):
    # 하이퍼파라미터 설정
    embedding_size1 = kwargs.get('embedding_size1', 2048)
    embedding_size2 = kwargs.get('embedding_size2', 2048)
    lstm_size = kwargs.get('lstm_size', 1024)
    dropout_ratio = kwargs.get('dropout_ratio', 0.2)

    # 인코더
    encoder_inputs = Input(shape=(max_text_len,))
    encoder_embedding = Embedding(corpus_size, embedding_size1, trainable=True, mask_zero=True)(encoder_inputs) # 패딩 마스킹
...
# lstm 레이어 추가
#################################### lstm 레이어 추가###########################################
    # encoder_lstm2 = Bidirectional(LSTM(lstm_size, return_state=True, return_sequences=True))
    # encoder_outputs, forward_h, forward_c, backward_h, backward_c = encoder_lstm2(encoder_outputs)

    # encoder_lstm3 = Bidirectional(LSTM(lstm_size, return_state=True, return_sequences=True))
    # encoder_outputs, forward_h, forward_c, backward_h, backward_c = encoder_lstm3(encoder_outputs)

    # encoder_lstm4 = Bidirectional(LSTM(lstm_size, return_state=True, return_sequences=True))
    # encoder_outputs, forward_h, forward_c, backward_h, backward_c = encoder_lstm4(encoder_outputs)
################################################################################################
    ...
    # lstm 레이어 추가
#################################### lstm 레이어 추가###########################################
    # decoder_lstm2 = LSTM(lstm_size*2, return_sequences=True, return_state=True)
    # decoder_outputs, _, _ = decoder_lstm2(decoder_outputs)

    # decoder_lstm3 = LSTM(lstm_size*2, return_sequences=True, return_state=True)
    # decoder_outputs, _, _ = decoder_lstm3(decoder_outputs)

    # decoder_lstm4 = LSTM(lstm_size*2, return_sequences=True, return_state=True)
...
def predict_from_seq(moonjang_, encoder_model, decoder_model, sets_for_predict):
    ...
    decoded_label = []
    init_out, init_h, init_c = states_value[0], states_value[1], states_value[2]
